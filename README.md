...
`LOCAL_FILES_ONLY=false` allows missing model weights (such as embeddings) to be downloaded when network is available. Set it to `true` for strictly offline/air-gapped runs after pre-downloading models.
